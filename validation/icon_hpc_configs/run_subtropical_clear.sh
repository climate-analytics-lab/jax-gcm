#!/bin/bash
#SBATCH --job-name=icon_subtropical_clear
#SBATCH --partition=compute
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --time=02:00:00
#SBATCH --mem=8GB
#SBATCH --output=subtropical_clear_%j.out
#SBATCH --error=subtropical_clear_%j.err

# Test Case: Subtropical Clear Sky
# Location: 30°N, 30°W (Azores High region)
# Period: 10 days starting July 15, 2020
# Purpose: Subsidence, clear-sky radiation, surface energy balance

echo "Starting ICON subtropical clear sky test case..."
echo "Test case: Subtropical Clear Sky"
echo "Location: 30°N, 30°W"
echo "Period: 10 days starting July 15, 2020"
...
export ICON_CASE="subtropical_clear"
...
# Create single column grid at 30°N, 30°W
lat = ${ICON_LAT}
lon = ${ICON_LON}

# Create minimal grid file
with nc.Dataset('scm_grid_subtropical.nc', 'w') as f:
    # Dimensions
    ...
    f.setncattr('grid_id', 'scm_subtropical')
...
# Run ICON
echo "Running ICON..."
${ICON_EXEC} subtropical_clear.nml
...
try:
    ...
    ds_phys = xr.open_dataset("subtropical_clear_physics_20200715T000000Z.nc")
    ds_rad = xr.open_dataset("subtropical_clear_radiation_20200715T000000Z.nc")
    ds_sfc = xr.open_dataset("subtropical_clear_surface_20200715T000000Z.nc")
    ds_conv = xr.open_dataset("subtropical_clear_convection_20200715T000000Z.nc")
    ds_turb = xr.open_dataset("subtropical_clear_turbulence_20200715T000000Z.nc")
    ...
    # Add metadata
    ds_validation.attrs['case_name'] = 'subtropical_clear'
    ds_validation.attrs['location'] = '30°N, 30°W'
    ds_validation.attrs['model'] = 'ICON'
    ds_validation.attrs['physics'] = 'ECHAM'
    
    # Save processed data
    output_file = "icon_validation_subtropical_clear.nc"
    ...
    print(f"Clear-sky SW radiation: {ds_validation.sw_flux_down.mean().values:.2f} W/m²")
    print(f"Surface energy balance: {(ds_validation.surface_sensible_flux + ds_validation.surface_latent_flux).mean().values:.2f} W/m²")
...
EOF
    ...
    echo "Output file: icon_validation_subtropical_clear.nc"
...
echo "Subtropical clear sky test case completed at: $(date)"
echo "Files created:"
ls -la icon_validation_subtropical_clear.nc subtropical_clear_*.nc 2>/dev/null || echo "No output files found"