...
- The target was not transformed, standardized, clipped, class-balanced, or
  site-weighted. One row therefore receives one vote; long site records contribute more than
  short records.
...
### Site terrain and one-step diagnostics
...
- (**TODO:** This seems bad, we should probably normalize?) There was no train-fitted scaling, normalization, imputation, dimensional
...
### 8.3 ERA5 atmospheric remapping
...
### 9.3 Fixed-total SPEEDY-ratio counterfactual

A 240-window 2011-2020 diagnostic tested the radiative component treatment. SPEEDY
separately diagnoses generic cloud `cloudc` and PBL stratocumulus `cloudstr`;
these are not a conserved observational partition. We derived its local ratio
`r = cloudstr/(cloudc+cloudstr)` and applied it to nested RH's unchanged total
cloud: `cloudc=(1-r)C_nested`, `cloudstr=r*C_nested`. Total cloud was preserved
to `5.96e-8` or better.

| Configuration | Cloud RMSE | RSUT RMSE (W m-2) |
| --- | ---: | ---: |
| Calibrated SPEEDY | 0.21074 | **40.55906** |
| Nested RH, one component | **0.19077** | 43.09910 |
| Nested RH, SPEEDY-like ratio | **0.19077** | 41.57207 |
| Nested RH, ratio plus SPEEDY cloud top | **0.19077** | 41.57026 |

The ratio improved RSUT RMSE in all 240 windows, removing 60.1% of the nested
RSUT penalty while retaining its better total-cloud RMSE. Cloud-top replacement
was negligible. Solar-weighted cloud RMSE and a common-shortwave-operator proxy
favored nested RH in 239 and 235 windows, respectively, so the RSUT penalty primarily comes from
discarding SPEEDY's two-component radiative treatment, not from cloud errors
occurring in weakly illuminated locations. The ratio remains a post-hoc SPEEDY
model diagnostic rather than evidence that its component split is physically
correct. Full results are in
`outputs/era5_partition_counterfactual_240window.json`.
