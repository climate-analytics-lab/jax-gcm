...
the v3 integrator with the exact legacy ``days % 365`` phase patched in
reproduces the preceding reference bit-for-bit (0.0 normalized RMS in every
field, measured against the reference of the physics it was merged onto),
while the Gregorian January-1 phase moves the winds by 7.5–9.7 % RMS. This
identifies the reference shift as the intended removal of #449's seven-day
epoch offset, rather than an integration or output-label change. Only that
affected reference was regenerated; its cross-runner tolerance remains 3%.
