...
| **4. Remove the legacy inside-RK path** | ⏳ todo | Delete `_step_tendencies`, `physics_forcing_eqn`, the `compose_equations([primitive, physics_forcing_eqn(d)])` line; delete the `use_op_split` flag (only one path now). Drop `get_empty_data` and the legacy `averaged_trajectory_from_step` once op-split is the only path. | `jcm/model.py`, `jcm/physics_interface.py`, `jcm/physics/composable_physics.py`. | Default path is op-split. PR #469's IMEX-substage cache-write gating is fully dead code by this point and is removed. The legacy `DiagnosticsCollector` accumulator path is gone — the op-split path's explicit pytree carry is the only one. |
