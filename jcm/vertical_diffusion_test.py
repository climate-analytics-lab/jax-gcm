...
class Test_VerticalDiffusion_Unit(unittest.TestCase):
    ...
    def test_get_vertical_diffusion_tend(self):
        ...
        self.assertTrue(np.allclose(utenvd, jnp.zeros_like(utenvd), atol=1e-4))
        self.assertTrue(np.allclose(vtenvd, jnp.zeros_like(vtenvd), atol=1e-4))
        self.assertTrue(np.allclose(ttenvd[0,0,:], jnp.array([ 2.78098357e-04,  1.39862334e-04,  8.50690617e-05,  3.73100450e-05,
        3.67983799e-06, -2.65383318e-05, -6.18272365e-05, -3.07837296e-04]), atol=1e-4))
        self.assertTrue(np.allclose(qtenvd[0,0,:], jnp.array([ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00,  0.00000000e+00, 9.99411916e-06,  7.24206425e-06,  1.30163815e-05, -4.72222083e-05]), atol=1e-4))