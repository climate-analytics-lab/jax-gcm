...
class Test_VerticalDiffusion_Unit(unittest.TestCase):
    ...
    def test_get_vertical_diffusion_tend(self):
        ...
        utenvd, vtenvd, ttenvd, qtenvd = vertical_diffusion.get_vertical_diffusion_tend(se, rh, qa, qsat, phi, icnv,
                                                                                     DiffusionConstants(), fsg, dhs, sigh)
        self.assertAlmostEquals(utenvd[20,20], np.asarray([0., 0., 0., 0., 0., 0., 0., 0.]))
        self.assertAlmostEquals(vtenvd[20,20], np.asarray([0., 0., 0., 0., 0., 0., 0., 0.]))
        self.assertAlmostEquals(ttenvd[20,20], np.asarray([2.78098343e-04,  1.39862327e-04,  8.50690984e-05,  3.73100365e-05,
        3.67983118e-06, -2.65383233e-05, -6.18272700e-05, -3.07837272e-04]))
        ##  ttenvd at the the second sigma level gives  inf, which might be due to the Step 3: Damping of super-adiabatic lapse rate
        self.assertAlmostEquals(qtenvd[20,20], np.asarray([ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00,  0.00000000e+00,
        9.99411857e-06,  7.24205452e-06,  1.30163929e-05, -4.72222055e-05]))
