...
from jcm.physics_interface import PhysicsState, Physics, PhysicsTendency, SpeedyPrimitiveEquations, get_physical_tendencies
from jcm.physics.speedy.physics_data import PhysicsData, PhysicsOutputData
...
class Model:
    ...
    def post_process(self, state: ModelState) -> Predictions:
        ...
        physics_data = state.diagnostics

        if self.physics.post_process:
            date=DateData.set_date(
                model_time = self.start_date + Timedelta(seconds=state.sim_time),
    ...
    def unroll(self, state: ModelState) -> tuple[ModelState, Predictions]:
