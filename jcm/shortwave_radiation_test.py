...
class TestShortWaveRadiation(unittest.TestCase):
    ...
    def test_clouds_gradients_isnan_ones(self): 
        """Test that we can calculate gradients of shortwave radiation without getting NaN values"""
        xy = (ix, il)
        xyz = (ix, il, kx)
        physics_data = PhysicsData.ones(xy,kx)  # Create PhysicsData object (parameter)
    ...
    def test_shortwave_radiation_gradients_with_realistic_values(self):   
        from datetime import datetime
        ...
        # date_data = DateData.set_date(model_time = Timestamp(delta=Timedelta(days=200, seconds=0)))
