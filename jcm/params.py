...
class SlaboceanModelParameters:
    ...
    tau_ocn      : jnp.ndarray # relaxation time of SST (unit: seconds)
    d_min    : jnp.ndarray # mixed layer thickness of low  latitude (unit: meter)
    d_max    : jnp.ndarray # mixed layer thickness of high latitude (unit: meter)
    thrsh     : float       # threshold beyond which the ocean is not active

    @classmethod
    def default(self):
        return SlaboceanModelParameters(
            tau_ocn = jnp.array(60 * 86400.0),
            d_min = jnp.array(40.0),
            d_max = jnp.array(120.0),
            thrsh   = 0.5,
        )
