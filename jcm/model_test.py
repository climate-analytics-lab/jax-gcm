...
class TestModelUnit(unittest.TestCase):
    ...
    def test_echam_model_default_statistics(self):
        """Run default ``echam_physics()`` and assert each var falls in its
        stored climatology band (mean ± 3·std).

        Mirrors :meth:`test_speedy_model_default_statistics`. Cross-hardware
        bit reproducibility is not achievable, so the test compares
        global-mean diagnostics against frozen climatology statistics
        rather than per-element trajectories. Stats live in
        ``jcm/data/test/echam/default_statistics.nc`` and are
        regenerated by ``jcm/data/test/echam/generate_default_stats.py``
        when the physics changes intentionally.
        """
        import xarray as xr
        from importlib import resources

        from jcm.data.test.echam.generate_default_stats import (
            default_echam_stat_vars,
            run_default_echam_model,
        )

        stats_file = (
            resources.files("jcm.data.test.echam") / "default_statistics.nc"
        )
        default_stats = xr.open_dataset(stats_file)

        # Run a single 5-day average (output_averages=True via save_interval).
        # The saved climatology band is computed from the same 5-day window
        # of daily-mean samples, so the test's 5-day mean equals the mean
        # of the 5 daily means by construction.
        _, predictions = run_default_echam_model(
            save_interval=5.0, total_time=5.0,
        )
        pred_ds = predictions.to_xarray()
        pred_ds_mean = pred_ds.isel(time=-1).mean(dim={"lon", "lat"})

        tol = 3  # tolerance in standard deviations
        for var in default_echam_stat_vars:
            lower = default_stats[f"{var}.mean"] - tol * default_stats[f"{var}.std"]
            upper = default_stats[f"{var}.mean"] + tol * default_stats[f"{var}.std"]
            assert ((lower <= pred_ds_mean[var]).all()) & (
                ...
                "regenerate jcm/data/test/echam/default_statistics.nc "
