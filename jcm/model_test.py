...
class TestOperatorSplitPhysics(unittest.TestCase):
    ...
    def test_op_split_step_matches_manual_strang(self):
        """The op-split single-step is a Strang split: physics is
        evaluated *once* on the input state and applied as two
        ½dt forward-Euler adds bracketing the full-dt dynamics
        step.

        Rebuild both the Strang reconstruction *and* the Lie-split
        alternative (single full-dt apply before dynamics) from the
        same primitives, then verify the actual step output is much
        closer to Strang than to Lie. Comparing distances rather than
        an absolute tolerance keeps the test robust to JIT-vs-eager
        arithmetic-ordering noise (which is much smaller than the
        ``O(dt · physics_tendency)`` Lie/Strang state separation).
        """
        import numpy as np
        from jcm.forcing import default_forcing
        ...
        forcing = default_forcing(model.coords.horizontal)
        step_fn = jax.jit(model._get_op_split_step_fn(forcing))
        x_step, _ = step_fn(initial_state, ps0)

        # Reconstruct the Strang and Lie outputs from the same primitives
        # the step uses internally.
        date = model._date_from_sim_time(initial_state.sim_time)
        forcing_now = forcing.select(date, calendar=model.calendar)
        dyn_tendency, _ = compute_physics_step(
            state=initial_state,
            dynamics=model.primitive,
            time_step=model.dt_si.m,
            physics=model.physics,
            forcing=forcing_now,
            terrain=model.terrain,
            physics_state=ps0,
        )
        dynamics_step = model._get_dynamics_step_fn()
        half_dt = 0.5 * model.dt

        x_strang = (
            dynamics_step(initial_state + half_dt * dyn_tendency)
            + half_dt * dyn_tendency
        )
        x_lie = dynamics_step(initial_state + model.dt * dyn_tendency)
        for f in model.filters:
            x_strang = f(initial_state, x_strang)
            x_lie = f(initial_state, x_lie)

        def total_l2(a, b):
            leaves_a = jax.tree_util.tree_leaves(a)
            leaves_b = jax.tree_util.tree_leaves(b)
            return float(sum(
                jnp.sum(jnp.square(la - lb))
                for la, lb in zip(leaves_a, leaves_b)
            ))

        d_step_strang = total_l2(x_step, x_strang)
        d_step_lie = total_l2(x_step, x_lie)
        d_strang_lie = total_l2(x_strang, x_lie)

        # Strang and Lie must actually differ — otherwise the test is
        # degenerate (zero physics tendency or trivial dynamics).
        self.assertGreater(
            d_strang_lie, 0.0,
            "Strang and Lie reconstructions are identical — pick a "
            "configuration that exercises the splitting choice.",
        )
        # The step output should be Strang to ≪ the Lie/Strang gap.
        # 1 % is generous given JIT-vs-eager rounding; in practice
        # this ratio is many orders of magnitude smaller.
        self.assertLess(
            d_step_strang, 0.01 * d_strang_lie,
            "op-split step is closer to the Lie reconstruction than"
            " to the Strang reconstruction — has the splitting"
            " scheme been changed back to a single full-step apply?"
            f" (d(step, Strang) = {d_step_strang:.3e}, "
            f"d(step, Lie) = {d_step_lie:.3e}, "
            f"d(Strang, Lie) = {d_strang_lie:.3e})",
        )
