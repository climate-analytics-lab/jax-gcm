...
from jax_solar import OrbitalTime, radiation_flux
...
# Max temperature change from convective adjustment in one timestep [K].
# Prevents runaway: full instant adjustment in one step can overshoot and destabilize.
MAX_CONVECTIVE_DT_PER_STEP = 5.0
...
def compute_saturation_humidity(temperature: jnp.ndarray, pressure: jnp.ndarray, relative_humidity: float = 0.75) -> jnp.ndarray:
    ...
    # Specific humidity from relative humidity: q = eps * RH * p_v_sat / (p - (1-eps)*p_v_sat)
    # with eps = R_d/R_v. Using alpha = eps*RH*p_v_sat/(p - p_v_sat) and q = alpha/(1+alpha)
    # is equivalent but singular when p_v_sat -> p; use proper formula for numerical stability.
    eps = R_d / R_v
    q_t = eps * relative_humidity * p_v_sat / (pressure - (1.0 - eps) * p_v_sat)
...
class RCESolver:
    ...
    def __init__(self, setup: RCESetup):
        ...
        _orbital_time = OrbitalTime.from_datetime(_date)
        _toa_flux = radiation_flux(
    ...
    def compute_radiation(self, state: RCEState) -> Tuple[jnp.ndarray, float]:
        ...
        tendencies, diagnostics = self.radiation_fn(
            ...
            surface_temperature=jnp.array(SURFACE_TEMPERATURE),
            surface_albedo_vis=jnp.array(self.setup.surface_albedo_vis),
            surface_albedo_nir=jnp.array(self.setup.surface_albedo_nir),
            surface_emissivity=jnp.array(self.setup.surface_emissivity),
            date=date,
    ...
    def compute_convective_adjustment(self, state: RCEState) -> jnp.ndarray:
        ...
        if jnp.any(unstable_mask):
            ...
            # Temperature change that would achieve target lapse rate in this layer
            temp_adjustment = (target_dT_dz - dT_dz) * state.layer_thickness
            # Limit per-step change so we relax toward moist adiabat over several steps
            # (avoids runaway from overshooting in one large dt)
            temp_adjustment = jnp.clip(
                temp_adjustment,
                -MAX_CONVECTIVE_DT_PER_STEP,
                MAX_CONVECTIVE_DT_PER_STEP,
            )
