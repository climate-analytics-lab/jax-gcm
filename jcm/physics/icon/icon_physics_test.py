...
class TestIconPhysicsIntegration(unittest.TestCase):
    ...
    def test_icon_physics_integration_3_timesteps(self):
        ...
        """
        # Create model with ICON physics for 3 timesteps
        # Using smaller time step and shorter total time for fast testing
        model = Model(
            save_interval=3,  # Save every 3 time steps
        ...
        self.assertIsNotNone(dynamics_predictions.surface_pressure, "surface_pressure should not be None")
        ...
        self.assertFalse(jnp.any(jnp.isnan(dynamics_predictions.surface_pressure)), 
                         "surface_pressure should not contain NaN values")
        ...
        # Surface pressure should be positive and in reasonable range (50000 - 120000 Pa)
        sp_min = jnp.min(dynamics_predictions.surface_pressure)
        sp_max = jnp.max(dynamics_predictions.surface_pressure)
        ...
        expected_time_steps = int(model.total_time / model.save_interval) + 1  # +1 for initial state
