...
from jcm.boundaries import default_forcing, BoundaryData
...
def create_test_boundaries(lon_points=96, lat_points=48):
    forcing = default_forcing(get_coords(nodal_shape=(lon_points, lat_points)).horizontal)
...
class TestOrographicCorrection:
    ...
    def test_humidity_horizontal_correction(self):
        ...
        forcing = create_test_boundaries(lon_points=96, lat_points=48)
        geometry = create_test_geometry(orography=True)
        ...
        qcorh = compute_humidity_correction_horizontal(forcing, tcorh, land_temp)
    ...
    def test_get_orographic_correction_tendencies(self):
        ...
        forcing = create_test_boundaries()
        geometry = create_test_geometry(orography=True)
        parameters = Parameters.default()
    ...
    def test_humidity_horizontal_correction_gradient_check(self):
        ...
        lon, lat = 96, 48
        test_forcing = create_test_boundaries(lon_points=lon, lat_points=lat)
        geometry = create_test_geometry()
        forcing = ForcingData.ones((lon, lat),
                                       fmask = test_geometry.fmask,
        ...
        def f(boundaries_f, tcorh, land_temp):
            return compute_humidity_correction_horizontal(forcing=convert_back(boundaries_f, boundaries), 
