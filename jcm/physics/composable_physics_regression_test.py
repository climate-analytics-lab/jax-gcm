...
IMPORTANT: a reference is only valid against CI if it is generated with the
**same jax/jaxlib (and Python)** that CI installs — a different jaxlib emits
different XLA CPU code and the 1-day integration amplifies the difference past
the tolerance above. Run ``./regenerate_regression_references.sh`` from the repo
root, which reproduces CI's environment (Python 3.11 + ``pip install -r
requirements.txt && pip install -e .``, see ``.github/workflows/run_test.yaml``)
before invoking the command above, rather than regenerating in an arbitrary dev
environment.
"""
