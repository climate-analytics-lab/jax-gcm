...
from jcm.physics.aerosol.ham.activation.arg import (
    _shape_coefficients,
    arg_activation,
)
from jcm.physics.aerosol.ham.activation.arg_term import ArgActivation
...
class ArgTermTest(unittest.TestCase):
    def _ham_state(self, nlev=3, ncols=2):
        from jcm.physics.aerosol.ham import MAM4_SPEC
        ...
        return HamAerosolState(
            ...
            mass_per_mode=jnp.full(shape, 1e-9),
            number_per_mode=jnp.full(shape, 1.0e8),  # kg^-1
        )
    ...
    def test_term_writes_activated_cdnc(self):
        ...
        diagnostics = {
            "_ham_state": self._ham_state(nlev, ncols),
