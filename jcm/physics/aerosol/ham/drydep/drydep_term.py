"""``HamDryDeposition`` — turbulent/Brownian dry removal at the surface.
...
from jcm.physics.aerosol.ham.drydep.resistances import deposition_velocity
...
from jcm.physics.aerosol.ham.sedimentation.sedi_term import stokes_velocity
...
class HamDryDeposition(PhysicsTerm):
    ...
    def __call__(self, state, diagnostics, forcing, terrain):
        ...
        for i, mode in enumerate(self._spec.modes):
            r_sfc = ham.r_wet[i, -1]
            v_grav = stokes_velocity(r_sfc, ham.rho[i, -1], t_sfc, p_sfc)
