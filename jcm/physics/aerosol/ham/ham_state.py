"""Inter-term aerosol diagnostic sub-struct (the ``_ham_state`` key).
...
key ``"_ham_state"`` (inter-term plumbing, auto-flattened to ``ham_state.*``
...
class HamAerosolState:
    ...
    mass_per_mode: jnp.ndarray    # total dry mass [kg/kg]   (n_modes, nlev, ncols)
    number_per_mode: jnp.ndarray  # number [kg^-1]           (n_modes, nlev, ncols)

    @classmethod
    def zeros(cls, nodal_shape, nlev, n_modes) -> "HamAerosolState":
        ...
        return cls(
            ...
            mass_per_mode=jnp.zeros(shape),
            number_per_mode=jnp.zeros(shape),
        )

    def copy(self, **kwargs) -> "HamAerosolState":
        data = {
            "r_dry": self.r_dry,
            "r_wet": self.r_wet,
            "rho": self.rho,
            "kappa": self.kappa,
            "mass_per_mode": self.mass_per_mode,
            "number_per_mode": self.number_per_mode,
        }
        data.update(kwargs)
        return HamAerosolState(**data)
