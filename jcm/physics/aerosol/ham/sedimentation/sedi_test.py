...
class SedimentationTermTest(unittest.TestCase):
    def _setup(self, nlev=4, ncols=2):
        ...
        ham = HamAerosolState(
            ...
            number_per_mode=jnp.full(shape, 1.0e8),
        )
        tracers = {}
    ...
    def test_term_produces_tracer_tendencies(self):
        from jcm.physics.aerosol.ham import MAM4_SPEC, mass_name

        state, diagnostics = self._setup()
        term = HamSedimentation()
    ...
    def test_grad_through_velocity_scale_finite(self):
        from jcm.physics.aerosol.ham.sedimentation.sedi_term import SedParameters
