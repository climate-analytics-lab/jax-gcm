"""``HamWetDeposition`` — in-cloud and below-cloud aerosol scavenging.
...
class HamWetDeposition(PhysicsTerm):
    ...
    requires: ClassVar[tuple[str, ...]] = (
        "_ham_state", "activated_fraction", "air_density", "layer_thickness",
    )
    ...
    def __call__(self, state, diagnostics, forcing, terrain):
        ...
        for i, mode in enumerate(self._spec.modes):
            rate_below = below_cloud_rate(
                precip_col, cloud_fraction, ham.r_wet[i], params,
            )
