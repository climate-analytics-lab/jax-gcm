"""Microphysics-core contract for the HAM aerosol harness.
...
needs, and each step writes the ``_ham_state`` diagnostic the downstream
harness terms read.
...
from jcm.physics.aerosol.ham.tracer_layout import tracer_specs
...
class ModalMicrophysicsTerm(PhysicsTerm):
    ...
    :meth:`__call__` to write the ``_ham_state`` diagnostic. The tracer set
    ...
    provides: ClassVar[tuple[str, ...]] = ("_ham_state",)
