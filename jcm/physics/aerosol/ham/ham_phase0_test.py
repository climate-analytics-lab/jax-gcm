...
from jcm.physics.aerosol.ham import (
    MAM4_SPEC,
...
class PlaceholderTermTest(unittest.TestCase):
    def test_term_smoke(self):
        ...
        self.assertIn("_ham_state", diag)
        self.assertEqual(diag["_ham_state"].r_dry.shape, (4, nlev, ncols))
        self.assertTrue(
            np.all(np.isfinite(np.asarray(diag["_ham_state"].r_wet)))
        )
