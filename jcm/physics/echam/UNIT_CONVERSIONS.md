# Unit Conversions in ECHAM Physics

This document describes the unit conversions between the JAX-GCM physics interface and ECHAM physics parameterizations.
...
## ECHAM Physics Expected Units

ECHAM physics modules expect all inputs in SI units:
...
All unit conversions are handled within the `EchamPhysics` class:
...
3. **Time Step**: 
   ...
   - Must set dt_conv parameter when creating EchamPhysics:
     ```python
     parameters = Parameters.default().with_convection(dt_conv=time_step_minutes * 60)
     physics = EchamPhysics(parameters=parameters)
     ```
...
```bash
python -m jcm.physics.echam.test_unit_conversions
```
