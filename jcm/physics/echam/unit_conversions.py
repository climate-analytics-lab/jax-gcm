"""Unit conversion utilities for ECHAM physics

This module ensures that physics state variables from the dynamics core
are properly converted to the units expected by ECHAM physics modules.
...
def prepare_physics_state_2d(state, echam_coords):
    """Prepare physics state with proper unit conversions for 2D (vectorized) format.

    This is used within EchamPhysics after state has been reshaped to [nlev, ncols].

    Args:
        state: PhysicsState in 2D format [nlev, ncols]
        echam_coords: EchamCoords object with cached coordinate data
    ...
    pressure_levels = echam_coords.calculate_pressure_full(surface_pressure_pa)
...
def prepare_physics_state_3d(state, echam_coords):
